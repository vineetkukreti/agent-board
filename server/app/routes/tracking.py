...
def _cwd_to_project_slug(cwd: str) -> str:
    ...
    Handles subdirs like server/, client/, src/ by going up to the project root."""
...
async def _resolve_sprint_id(
    ...
    # Check cwd AND parent dirs (agents often run from subdirs like server/ or client/)
    if cwd:
        from pathlib import Path as FsPath
        search_dirs = [FsPath(cwd)]
        # Walk up to 3 parents to find sprint files (e.g., server/ → project root)
        p = FsPath(cwd).parent
        for _ in range(3):
            if p == p.parent:
                break
...
async def auto_register(
    ...
    if body.task_description:
        # Extract a clean short title — first sentence or first 80 chars
        raw = body.task_description.strip().split('\n')[0].strip()
        # Remove common prefixes like "You are X." or "Your task is to"
        import re
        ...
        cursor = await db.execute(
            ...
            (title, f"Auto-created from agent session.\nAgent: {body.agent_name}\nCWD: {body.cwd}", project_id, agent_id, sprint_id, now, now),
        )
