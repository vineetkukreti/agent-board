...
Two auth paths:
  ...
  2. Admin auth  — Authorization: Bearer <session_token>
                   Validated against the admin_sessions table in SQLite.
...
def _session_db_path() -> str:
    db_path = os.getenv("DB_PATH", "./data/agent-board.db")
    p = Path(db_path)
...
def _session_db() -> sqlite3.Connection:
    db = sqlite3.connect(_session_db_path(), timeout=3)
...
def create_session(admin_id: int, username: str, role: str = "admin") -> str:
    """Create a new admin session token, persist to SQLite, and return the token."""
    token = secrets.token_urlsafe(32)
    db = _session_db()
    try:
        db.execute(
            "INSERT INTO admin_sessions (token, admin_id, username, role) VALUES (?, ?, ?, ?)",
            (token, admin_id, username, role),
        )
        db.commit()
    finally:
        db.close()
...
def get_session(token: str) -> dict[str, Any] | None:
    """Look up a session token in SQLite. Returns session dict or None."""
    db = _session_db()
    try:
        row = db.execute(
            "SELECT admin_id, username, role FROM admin_sessions WHERE token = ?",
            (token,),
        ).fetchone()
        if row:
            return {"admin_id": row["admin_id"], "username": row["username"], "role": row["role"]}
        return None
    finally:
        db.close()


def update_sessions_for_user(user_id: int, username: str | None = None, role: str | None = None) -> None:
    """Update active sessions when a user's role or username changes."""
    db = _session_db()
    try:
        if role is not None:
            db.execute("UPDATE admin_sessions SET role = ? WHERE admin_id = ?", (role, user_id))
        if username is not None:
            db.execute("UPDATE admin_sessions SET username = ? WHERE admin_id = ?", (username, user_id))
        db.commit()
    finally:
        db.close()
...
def delete_sessions_for_user(user_id: int) -> None:
    """Invalidate all sessions for a deleted user."""
    db = _session_db()
    try:
        db.execute("DELETE FROM admin_sessions WHERE admin_id = ?", (user_id,))
        db.commit()
    finally:
        db.close()
...
async def require_auth(
    ...
    # 2. Try admin session token (SQLite-backed)
    session = get_session(raw_key)
