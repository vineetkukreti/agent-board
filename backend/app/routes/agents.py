...
async def heartbeat(
    ...
    await db.execute(
        "UPDATE agents SET last_seen_at = datetime('now'), "
        "status = CASE WHEN status IN ('offline', 'idle') THEN 'active' ELSE status END, "
    ...
    try:
        ...
        await broadcast("agent.updated", {"agent_id": agent_id})
    except Exception:
        pass
...
@router.get("/{agent_id}/sparkline")
async def get_agent_sparkline(
    agent_id: int,
    db: aiosqlite.Connection = Depends(get_db),
    _current=Depends(require_auth),
):
    """Get 24-hour session activity sparkline for an agent."""
    cursor = await db.execute(
    ...
    hours = [hour_map.get(f"{h:02d}", 0) for h in range(24)]
    total = sum(hours)
    return {"hours": hours, "total_sessions_24h": total}
