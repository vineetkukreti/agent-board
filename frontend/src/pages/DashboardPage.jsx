...
import {
  ...
  CheckCircle2, Circle, XCircle, Pause, Zap, DollarSign, Layers,
} from 'lucide-react'
import { useDashboard } from '../hooks/useDashboard'
import { useTrackingOverview } from '../hooks/useTracking'
...
export default function DashboardPage() {
  ...
  const { data: tracking, isLoading: trackingLoading } = useTrackingOverview()
        ...
        {trackingLoading ? (
          ...
          <>
            <StatCard
              icon={<Zap size={20} />}
              label="Tokens Today"
              value={((tracking?.today?.input_tokens_today ?? 0) + (tracking?.today?.output_tokens_today ?? 0)).toLocaleString()}
              color="#6366f1"
            />
            <StatCard
              icon={<DollarSign size={20} />}
              label="Cost Today"
              value={`$${(tracking?.today?.cost_today ?? 0).toFixed(4)}`}
              color="var(--success)"
            />
            <StatCard
              icon={<Layers size={20} />}
              label="Active Sessions"
              value={tracking?.active_sessions ?? 0}
              color="var(--accent)"
            />
            <StatCard
              icon={<DollarSign size={20} />}
              label="Total Cost"
              value={`$${(tracking?.total_cost_usd ?? 0).toFixed(4)}`}
              color="var(--warning)"
            />
          </>
        )}
